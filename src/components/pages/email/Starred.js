...
import { getStarredMessages } from '../../../actions';
...
class Starred extends Component {
    ...
    state = {
        ...
        _pageNavigation: [{ label: 'Dashboard', to: '/' }, { label: 'Email', to: '/email/inbox' }, { label: 'Starred' }]
    }

    componentDidMount() {
        this.setState({ currentPage: 1 }, () => {
            this.props.getStarredMessages(this.state.currentPage);
        })
    } 
    ...
    render() {
        return(
            <GenericPage title={I18n.t('pages.starred.title')} pageContentClasses="emails-page-content" pageNav={ this.state._pageNavigation }>
...
const mapStateToProps = ({ starredMessages }) => {
    return { 
        messages: starredMessages.messages || [],
        total: starredMessages.total || 0,
        currentPage: starredMessages.currentPage || 1,
        totalPages: starredMessages.totalPages || 1
    }
}

Starred.propTypes = {
    messages: PropTypes.array,
    getStarredMessages: PropTypes.func
}

export default connect(mapStateToProps, { getStarredMessages })(Starred);
